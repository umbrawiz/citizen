...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    ...
                    <button data-toggle="modal" data-target="#add-ward" class="btn-success"><i class="fa fa-plus"
        ...
        {{-- Modal Add --}}
        <div>
            @include('admin.wards.add')
        </div>

        {{-- Modal Edit --}}
        <div>
            @include('admin.wards.edit')
        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#dataTables-ward').DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(id) {
                            return `
                                <button data-id="${id}" id="button-edit-ward" data-toggle="modal" data-target="#edit-ward" class="btn-primary">
                                    <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                </button>
                                <button data-id="${id}" id="button-delete-ward" class="btn-danger">
            ...
            $("#wrapper").on('click', '#button-edit-ward', function() {

                const id = $(this).data('id');

                $.ajax({
                    url: '{{ route('api.ward.edit') }}',
                    type: 'GET',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.status == '200') {
                            $('#edit-ward input[name="name"]').val(response.data.name);
                            $('#edit-ward input[name="code"]').val(response.data.code);
                            $('#edit-ward input[name="id"]').val(id);
                        }
                    }
                });
            });

            $("#wrapper").on('click', '#button-delete-ward', function() {
                const id = $(this).data('id');

                $.ajax({
                    url: '{{ route('api.ward.delete') }}',
                    type: 'DELETE',
                    data: {
                        id: id
                    },
