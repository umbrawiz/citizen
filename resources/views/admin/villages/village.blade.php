...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Thông tin Ấp/Làng
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#wrapper").on('click', '#button-edit-village', function() {

                const id = $(this).data('id');

                $.ajax({
                    url: '{{ route('api.village.edit') }}',
                    type: 'GET',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.status == '200') {
                            $('#edit-village input[name="name"]').val(response.data.name);
                            $('#edit-village input[name="code"]').val(response.data.code);
                            $('#edit-village input[name="id"]').val(id);
                        }
                    }
                });
            });

            $("#wrapper").on('click', '#button-delete-village', function() {
                const id = $(this).data('id');

                $.ajax({
                    url: '{{ route('api.village.delete') }}',
                    type: 'DELETE',
                    data: {
                        id: id
                    },
