...
<body>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $.ajax({
                url: '{{ route('api.sum.declaration.province') }}',
                type: 'GET',
                headers: {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function(code) {
                            return `
                                <button data-code="${code}" id="button-show-province">
            ...
            $("#wrapper").on('click', '#button-show-province', function() {

                const code = $(this).data('code');

                $.ajax({
                    url: '{{ route('api.show.declaration.province') }}',
                    type: 'GET',
                    data: {
                        code: code
                    },
                    ...
                    success: function(response) {
                        if (response.status == '200') {
                            window.history.pushState('', '', '/show-province?code='+code);
                            $('#page-wrapper').html(response.data);
                            $('#dataTables-show-province').DataTable();
                        }
                    }
                });
            });
