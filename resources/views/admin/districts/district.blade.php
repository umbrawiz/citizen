...
<body>
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    ...
                    <button data-toggle="modal" data-target="#add-district" class="btn-success"><i class="fa fa-plus"
                            aria-hidden="true"></i></button>
                    <!-- /.col-lg-12 -->
                    <table class="table table-striped table-bordered table-hover" id="dataTables-district">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Tên</th>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#dataTables-district').DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(id) {
                            return `
                                <button data-id="${id}" id="button-edit-district" data-toggle="modal" data-target="#edit-district" class="btn-primary">
                                    <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                </button>
                                <button data-id="${id}" id="button-delete-district" class="btn-danger">
            ...
            $("#wrapper").on('click', '#button-edit-district', function() {

                const id = $(this).data('id');

                $.ajax({
                    url: '{{ route('api.district.edit') }}',
                    type: 'GET',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.status == '200') {
                            $('#edit-district input[name="name"]').val(response.data.name);
                            $('#edit-district input[name="code"]').val(response.data.code);
                            $('#edit-district input[name="id"]').val(id);
                        }
                    }
                });
            });

            $("#wrapper").on('click', '#button-delete-district', function() {
                const id = $(this).data('id');

                $.ajax({
                    url: '{{ route('api.district.delete') }}',
                    type: 'DELETE',
                    data: {
                        id: id
                    },
